...
% if Variables.WeirdPlate==true
% %  if strcmp(Condition.ConditionName, 'Chow')   
% %  Variables.FoodBR=false;
% %  else
% %  Variables.FoodBR=true;
%  end
...
DistanceTLplate2TLCorner= sqrt((Condition.Arena.top_left(1)-TLBox_coords(1)).^2 + (Condition.Arena.top_left(2)-TLBox_coords(2)).^2);
DistanceBRplate2TLCorner= sqrt((Condition.Arena.top_left(1)-BRBox_coords(2)).^2 + (Condition.Arena.top_left(2)-BRBox_coords(2)).^2);
DistanceTLplate2BRCorner= sqrt((Condition.Arena.bottom_right(1)-TLBox_coords(1)).^2 + (Condition.Arena.bottom_right(2)-TLBox_coords(2)).^2);
DistanceBRplate2BRCorner= sqrt((Condition.Arena.bottom_right(1)-BRBox_coords(2)).^2 + (Condition.Arena.bottom_right(2)-BRBox_coords(2)).^2);
Diff1=DistanceBRplate2BRCorner-DistanceTLplate2TLCorner;
Diff2=DistanceTLplate2BRCorner-DistanceBRplate2TLCorner;
if Diff1>1 && Diff2>1
BottomRightPlate=true; %BR plate is in the wrong location
elseif  Diff1<1 && Diff2<1
BottomRightPlate=false;  %TL plate is in the wrong location
else
   disp('Unable to determine plate location')   
end
% deal with a wrong plate assingment
XaxisDistance=abs(Condition.Arena.bottom_right(1)-Condition.Arena.bottom_left(1));
Distance2MoveX=XaxisDistance*6/100;
YaxisDistance=abs(Condition.Arena.bottom_right(2)-Condition.Arena.top_right(2));
Distance2MoveY=YaxisDistance*13/100;
if Condition.PlateDistance<150
if BottomRightPlate
BRBox_coords=[(Condition.Arena.bottom_right(1)-Distance2MoveX),(Condition.Arena.bottom_right(2)-Distance2MoveY)];
Condition.Center_BRBox=BRBox_coords;
else
TLBox_coords=[(Condition.Arena.top_left(1)+Distance2MoveX),(Condition.Arena.top_left(2)+Distance2MoveY)];
Condition.Center_TLBox=TLBox_coords;
end
end % if
% %% enter manually if its off
% BRBox_coords=[200.88 100];
% TLBox_coords=[];
title('Select BR plate')
[x, y] = ginput(1); % Use 1 for a single point; you can specify more points as needed
...
min_turn_duration = 1; % Minimum number of consecutive frames for a sustained turn
