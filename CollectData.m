...
if isempty(Condition.VideoTTLType) % not video TTL found, set to deafult number 20
Condition.VideoTTLType=20;
end
 % try to find the Laser TTL (it is given in a constant frequency of 1/2/5 hz)
for i=1:length(TTLtypesfound)
tempTTLtimestamps=(Condition.TimeStamps_events(Condition.TTLs==TTLtypesfound(i))-Condition.TimeStamps_events(1))/1000000;
TimestampDiffrence=[tempTTLtimestamps(2:end),tempTTLtimestamps(end)]-tempTTLtimestamps(1:end); 
if round(median(TimestampDiffrence(1:end-1)),2)==0.5 || round(median(TimestampDiffrence(1:end-1)),2)==1 || round(median(TimestampDiffrence(1:end-1)),2)==0.25% if its 0.25/0.5/1 sec apart (1/2/5hz)
...
%% check unit quality:
try
[Condition] = Clustering(Variables,Condition); 
catch
Condition.L_ratio=nan; 
Condition.isolation_distance=nan;
end
try Units_in_Condition(Unit2Screen, Index) = {Condition.L_ratio}; catch end
Units_in_Condition_Titles(Unit2Screen, Index) = {[char(Condition.ConditionName),'.L_ratio']};Index=Index+1;
try Units_in_Condition(Unit2Screen, Index) = {Condition.isolation_distance}; catch end
Units_in_Condition_Titles(Unit2Screen, Index) = {[char(Condition.ConditionName),'.isolation_distance']};Index=Index+1;
end
